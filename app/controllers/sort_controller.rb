  ...
  def weliike_sort
    ...
    respond_to  do |format|
      if @sort_setting.sort_by == "Recent"
         ...
         @user_entities_with_sub_category_and_city = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:city => @sort_setting.narrow_by_city, :sub_category => @sort_setting.narrow_by_sub_category).order_by(:created_at => "desc").group_by { |t| t.api_id }
         @user_entities_with_only_city = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:city => @sort_setting.narrow_by_city).group_by { |t| t.api_id }
         @user_entities_with_only_sub_category = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:sub_category => @sort_setting.narrow_by_sub_category).order_by(:created_at => "desc").group_by { |t| t.api_id }
         @user_entity_with_recent = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).order_by(:created_at => "desc").group_by { |t| t.api_id }#.uniq{|p| p.entity_name}
         if @sort_setting.narrow_by_sub_category.empty? && @sort_setting.narrow_by_city.empty?
          ...
          @user_entiy_for_api = UserEntity.in(:api_id => @poster, :id => @posters).uniq{|p| p.api_id}
               @user_entiy_for_api.each do |user_entity|
                   @post = Post.in(:user_entity_id => @posters).where(:api_id => user_entity.api_id).last
                   if @post.present?
                      if user_entity.user_id == params[:user_id] && user_entity.rating_count.nil?
                            @ratings = Rating.where(:user_entity_id =>user_entity.id )
                             if @ratings.present?
       ...
       if !@final_data.blank?
         ...
         @user_entities_with_sub_category_and_city = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:city => @sort_setting.narrow_by_city, :sub_category => @sort_setting.narrow_by_sub_category).order_by([:rating_count,:desc]).group_by { |t| t.api_id }
         @user_entities_with_only_city = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:city => @sort_setting.narrow_by_city).order_by([:rating_count,:desc]).group_by { |t| t.api_id }
         @user_entities_with_only_sub_category = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).in(:sub_category => @sort_setting.narrow_by_sub_category).order_by([:rating_count,:desc]).group_by { |t| t.api_id }
         @user_entity_with_recent = UserEntity.where(:is_active => "true").in(:user_category_id => @cate).order_by([:rating_count,:desc]).group_by { |t| t.api_id }#.uniq{|p| p.entity_name}
         if @sort_setting.narrow_by_sub_category.empty? && @sort_setting.narrow_by_city.empty?
              @user_entity_with_recent.each do | (key, value) |
                   value.each do |user_entity|
                   @user_category = UserCategory.where(:id => user_entity.user_category_id)
       ...
       if !@final_data.blank?
        ...
        userss = Array.new
                  ...
                  @category_relations.push(params[:user_id])
                    ...
                    @cate.each do |catess|
                           if @sort_setting.narrow_by_sub_category.empty? && @sort_setting.narrow_by_city.empty?
                              @user_entity_with_proxy = UserEntity.near(center, 10000000).and.where(:user_category_id => catess).in(:user_id => userss).group_by { |t| t.api_id }
                            ...
                            @user_entities_proxy_all = UserEntity.near(center,10000000).and.where(:user_category_id => catess).in(:user_id => userss,:sub_category => @sort_setting.narrow_by_sub_category).group_by { |t| t.api_id }
